...
## Database (Room, schema version 6)
...
Migrations: `MIGRATION_1_2`, `MIGRATION_2_3`, `MIGRATION_3_4`, `MIGRATION_4_5`, `MIGRATION_5_6` registered in `DatabaseModule`; keep all schema JSON exports under `app/schemas/` in sync with `Migrations.kt` when bumping the version. `MIGRATION_4_5` rebuilds `drug_stock_batches` (SQLite can't relax a column's `NOT NULL` via `ALTER TABLE`) to make strength nullable, and adds `durationOccurrences` to `scheduled_intakes`. `MIGRATION_5_6` adds the units-before low-stock reminder columns.
