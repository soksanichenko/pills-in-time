...
- **Times of day**: any number of times, each with its own dose — specified either as a plain **unit count** (e.g. "1 tablet") or as **active-substance strength** (e.g. "20 mg"). For a strength dose that could be made up of more than one combination of on-hand strengths (e.g. two 10 mg tablets vs. one 20 mg), you pick which combination it's fixed to right here, once — if there's only one possible combination, it's picked automatically and there's nothing to choose. That choice is what later drives real stock deduction (see below) — logging a dose never re-asks which tablets it came from. If the drug has more than one supply on hand and at least one time uses a plain unit count, you can similarly pin the whole period's consumption to one specific supply instead of drawing first-in-first-out across all of them. One time slot per period can instead be **session-based** — for a dose with no fixed clock time, like eye drops needed every hour while awake, or a plain "3 times a day": tap the Fixed time/Session-based toggle on that time, pick either "every N hours" or "N times a day", and set what time of day the app should start prompting you to begin (see Notifications below for how it runs).

An ongoing or upcoming period can be **stopped** or **paused** directly from its card, without opening the edit form: Stop ends it immediately, even if today's own dose hasn't been taken yet — any of today's already-posted reminders for it are dismissed right away and the period reads as inactive from today on. Pause suspends it — no reminders, no missed marks, no stock consumed — for a chosen number of days, a chosen number of doses, or indefinitely until you tap Resume. A period with a session-based time also gets a **Day started** / **Going to sleep** button here, for starting or ending that day's session by hand instead of waiting for (or reacting to) its notification. The card for whichever period is currently active is highlighted, and periods that have already ended collapse under a "Past periods" spoiler (tap to expand) so a long history doesn't clutter the list.
...
- **Session-based reminders**: for a period's session-based time (see Periods above), a persistent "Start your day?" prompt appears from your chosen start-of-day time and stays up until you tap it. From there, an "every N hours" session sends a regular, dismissible reminder for each dose (Took it / Skipped / Going to sleep) instead of a notification that sits in the shade all day; an "N times a day" session instead keeps one persistent "N of M taken today" status visible until you've logged them all, or until you end the day early. Going to sleep from either notification asks you to confirm first, since it sits right next to Took it/Skipped with no undo. Each of the three (start prompt, hourly reminder, N-times-a-day status) has its own notification channel, so you can mute any one of them independently of your regular medication reminders.
...
- Android Studio with AGP 9.3.2 / Gradle
- JDK 11+
