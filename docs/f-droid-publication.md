...
1. **Git release tags** — none exist yet (`git tag -l` is empty). F-Droid builds a specific `versionCode`/`versionName` off a specific tag or commit, so releases need to be tagged (e.g. `v1.0` for the current `versionCode 31`).
